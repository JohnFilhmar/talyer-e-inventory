...
/**
 * UpdatePaymentModal component
 *
 * Modal for updating payment information on a sales order.
...
export const UpdatePaymentModal: React.FC<UpdatePaymentModalProps> = ({
  ...
  const [amountPaid, setAmountPaid] = useState<string>(order.payment.amountPaid.toString());
  ...
  const derivedPaymentStatus = useMemo((): PaymentStatus => {
  ...
  // Don't allow payment updates for cancelled or refunded orders
  const isPaymentLocked = order.status === 'cancelled' || order.payment.status === 'refunded';
            ...
            {isPaymentLocked ? (
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                <p className="text-sm text-yellow-700 dark:text-yellow-400">
                  Payment cannot be updated for {order.status === 'cancelled' ? 'cancelled' : 'refunded'} orders.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    >
                      {PAYMENT_STATUS_OPTIONS.find(opt => opt.value === derivedPaymentStatus)?.label || derivedPaymentStatus}
                    </span>
                  </div>
                </div>
                ...
                {order.status === 'completed' && derivedPaymentStatus === 'paid' && order.payment.status !== 'paid' && (
                  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                    <p className="text-sm text-blue-700 dark:text-blue-400">
                      <strong>Note:</strong> Since this order is completed, marking it as paid will create a transaction record.
                    </p>
                  </div>
                )}
              </>
